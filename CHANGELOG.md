...
- **`base_url` configuration** — `Coolhand(base_url=...)` and `COOLHAND_BASE_URL` env var allow redirecting all API calls to a self-hosted or staging backend. Defaults to `https://coolhandlabs.com` when unset. `https://` is required; `http://localhost` and `http://127.0.0.1` are permitted for local development. Trailing slashes are normalized automatically. (#21)
